...
let
  ...
  cfg = config.grimoire.windowManager.niri;
in
{
  options.grimoire.windowManager.niri = {
    enable = lib.mkEnableOption "Enable niri window manager";
  };
};
  config = lib.mkIf config.grimoire.profiles
  ...
  config = lib.mkIf cfgwindowManager{
    services = {
      greetd = {
        enable fg.windowManager   restart = true;
